...
void Game::getInput()
{
	...
	else if (command == "open")
	{
		...
		if (box != NULL)
		{
			command.clear();
			ss >> command;
			if (command == "with") // || if (command == ("with"||"using"))
			{
				command.clear();
				ss >> command;
	...
	else if (command == "use")
	{
		...
		gameover = true;
	else if (command == "pack")//else if(command==("pack"||"Pack"||"pak"||"Pak"||"inventory"||"Inventory"||"inv"||"Inv"))
	...
	else if (command == "talk")//else if(command== ("talk"||"speak"))
	{
		command.clear();
		ss >> command;
		if (command.length() > 0)
	...
	else if (command == "poop") //else if(command == "poop"||"Poop")
		exCout << "ewgross\n";
	else
